...
const BoxLayout = (props) => {
	...
	const [fan, setFan] = useState({status: ''})
	const [data, setData] = useState();
	console.log(fan)
	React.useEffect(()=>{
 	fetch('https://3f12-1-53-54-255.ngrok-free.app/getLastData/v1', {
			method: 'GET',
			headers: {
	...
	React.useEffect(()=>{
	fetch('https://3f12-1-53-54-255.ngrok-free.app/set_mode/fan', {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
	...
	return (
		<View style={[styles.box, { backgroundColor: bgcolor }]}>
			...
			<View style={styles.info}>
				...
				<TouchableOpacity style={styles.button} onPress={() => {setState(!state), setFan({...fan, status: state?'OFF':'ON'})}}>
					<View style={styles.line}>
